...
`샛별`의 이야기는 완성된 대사를 seed에 저장하지 않는다. 이야기 템플릿을 선택하면 AI가
1일차 첫 4페이지를 생성하고 4페이지에서 음성 분기 입력을 요구한다. 첫 선택 뒤 5페이지를
이어 만들고 9페이지에서 두 번째 음성 분기를 받은 뒤, 10페이지로 그날 이야기를 마친다.
이 구조를 10일 동안 반복해 총 100페이지가 되었을 때만 이야기를 완료한다. 음성에서 인식한
선택 문장은 다음 사건과 그날 결말에 명시적으로 반영된다.
