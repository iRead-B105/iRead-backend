...
두 학습자의 일일 커리큘럼은 실제 자동 생성 정책과 동일하게 훈련 5개로 구성한다.
`샛별`의 첫 훈련은 기존 demo에 없던 `그림과 문장 연결하기`이며, 저장된 `imagePrompt`를
Mock AI가 이미지 URL로 변환하므로 학습자 App에서 이미지 선택 화면까지 확인할 수 있다.
`demo` 프로필로 Backend를 재시작하면 학습자 App 시연용인 `샛별`과 `한결`의 현재 훈련
진행 상태와 음성·시선 시도 데이터가 초기화되어 첫 훈련부터 다시 테스트할 수 있다.
